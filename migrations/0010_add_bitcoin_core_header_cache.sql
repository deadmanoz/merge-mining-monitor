...
-- current synced-tip horizon used to bound timestamp-based classification.
...
COMMENT ON TABLE bitcoin_core_header IS
  'Sparse canonical Bitcoin headers fetched from the required Core node: each difficulty-epoch boundary and the current synced-tip horizon.';
