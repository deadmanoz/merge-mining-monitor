...
const AUXPOW_HELP = {
  ...
  child_time: {
    ...
    body: [
      ...
      "That cadence is what makes a negative offset ordinary, but it does not make the offset a measurement of template age: only the two claims are stored, and nothing records when a template was built. Reading the gap as roughly the Bitcoin block interval takes a further assumption nothing here carries, that the pool rolled nTime forward while reusing the template. Had it instead fixed both at job creation, the same cadence would show an offset near zero while the template went on aging, so the gap is neither a floor nor a ceiling on how stale the template really was. A positive offset is different: it is an ordering disagreement worth investigating, though what it lies between depends on where this stamp came from, and the bar is a verified commitment rather than a stored header. Where the commitment was checked, as in a live Namecoin-family capture carrying its AuxPoW proof or an Elastos reconstruction verified against the commitment, the block commits to child data stamped later than its own header time and the disagreement is internal to the block. A header alone does not clear that bar, since historical imports store no AuxPoW proof. Where it is source-reported, as for RSK and Hathor capture, which take the chain's RPC timestamp without checking it against the merge-mining commitment, it says only that the reported child time exceeds the Bitcoin header time. Either way it is not proof that a clock was wrong. The child chains tolerate stamping ahead within their future-drift allowance; the Bitcoin nTime need not have been chosen after the child data either, since a pool can carry an nTime picked for an earlier base template while rebuilding the coinbase and merkle root around a newer child commitment. What the child-first dependency fixes is only that the child data was settled before the hashing, not the order in which the two numbers were chosen.",
