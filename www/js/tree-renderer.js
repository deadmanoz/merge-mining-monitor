import { applyTreeHighlight } from "./controls.js?v=0.7.10";
import { $, BLOCK_H, BLOCK_W, chainColor, chainDisplayName, EDGE_KINDS, EDGE_LEGEND, esc, KINDS, ORPHAN_LEGEND, state } from "./frontend-state.js?v=0.7.10";
import { DEFAULT_TREE_LAYOUT, layoutTreeNodes } from "./tree-layout.js?v=0.7.10";
...
function nodeFillVar(node) {
  const kind = node?.kind || "unknown";
  return kind === "unknown"
    ? ORPHAN_FILL_VAR[node?.btc_orphan_class] || "var(--orphan-pending)"
    : `var(--${kind})`;
}
