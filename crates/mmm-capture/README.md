...
| `btc_orphan` | The pure BTC orphan classifier: maps a Core-attested-absent, PoW-valid `unknown` parent into strict/weak/excluded, or pending when the injected Core-header cache cannot yet decide. The Core non-membership gate lives in the read model, not here. |
| `nbits_table` | Builds strict (BIP34-height) and weak (timestamp) contamination verdicts from the injected sparse Core header cache. It contains no embedded Bitcoin chain history. Horizon overruns hold capture rather than misclassify. |
...
| `source_registry` | `SOURCE_REGISTRY` defines every `source` row (permanent id/code/chain/kind/lifecycle), its child-target location, and the `*_SOURCE_CODE` constants. Bound bidirectionally to the producers' `chains::spec` table by a conformance test; feature-gated `generate` deterministically emits the seed SQL and frontend JS. |
