...
Fractal, Qbit) is a spec row served by the shared `chains::auxpow_family` runner
over the shared `chains::bitcoind_rpc` client, with no capture module of its
own; a second wire format inside that family gets one adapter file under
`chains/auxpow_family/` (Qbit), not a sibling chain module. A genuinely
...
|--------|----------------|
| `chains::spec` | The static `CHAINS` table: per chain a `ChainId`, env prefix, activation floor, poller/reorg defaults, optional `FamilySpec` (auth, fetch strategy, child payout, repair scope, malformed policy), and an optional `RangeCap`. The producer command facade resolves a spec row, then dispatches by `ChainId`. |
