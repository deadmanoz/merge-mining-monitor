...
`/sources`, `/competitions`, and `/version`), their query params, the success
envelope, the
...
| `projection/shared` | Multi-family infrastructure: `mod.rs` holds the shared row types, per-hash proof/observation loaders, row mappers, hash display/parse, cross-endpoint DTOs (`PoolObject`, `SourceSummary`, `ProofState`, `ChildChainEvidence`, `TreeCompetition`), and the declared-dup `load_strict_bip34_height`; `competition.rs` is the single definition of a stale-vs-canonical competition (the join, the kind/height predicates, and the overflow-guarded header-time delta) shared by the tree, block, and competitions projections; `backbone.rs` is the self-contained Bitcoin Core backbone-coverage subsystem (window completeness + the `backbone_unsynced`/`backbone_conflict` checks). |
...
| `projection/competitions` | Every stale-vs-canonical competition for `/api/v1/competitions`: a full-table read of the shared competition query, height-then-display-hash ordered, with `sources[]` matching `source_summary_for_block` (including the synthetic Bitcoin source). Served unaggregated so the client bins and filters locally. |
