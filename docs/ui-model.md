...
blocks, backed by `/api/v1/navigator/error-block`. The readout uses the same
height-plus-place form as "Latest stale" because each item is a single block on
the height axis. Ordering is newest-first
