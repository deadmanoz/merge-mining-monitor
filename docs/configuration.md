...
| `BITCOIN_RPC_URL` / `BITCOIN_RPC_USER` / `BITCOIN_RPC_PASSWORD` | Required Bitcoin Core classifier and header source for capture, import, and reconciliation commands. Each command refreshes the persisted Core-header cache before work begins. |
